...
class ActionsQueue {
  ...
  private queue: { id: number; action: () => Promise<any>; description: string }[] = [];
  private isProcessing = false;
  private actionCounter = 0;

  private constructor() {
    log("SERVER", "🔄 ActionsQueue initialized - Rate limiting protection active");
  }
  ...
  private async processQueue() {
    ...
    while (this.queue.length > 0) {
      ...
      try {
        log("SERVER", `🚀 [Queue] Executing Action #${actionData.id}: ${actionData.description}`);
        await actionData.action();
      } catch (error: any) {
        log("SERVER", `❌ [Queue] Action #${actionData.id} (${actionData.description}) failed: ${error.message}`);
      }

      // If there are more items, apply random delay (1s to 2.5s)
      if (this.queue.length > 0) {
        log("SERVER", `⏳ [Queue] Rate limiting: Waiting ${delay}ms before next action...`);
