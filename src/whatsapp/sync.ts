...
const getCacheFile = () =>
    join(getDataDir(), "baileys_auth", "store_cache.json");
...
export class WhatsAppSync {
        ...
        try {
            if (existsSync(getCacheFile())) {
                ...
                if (cache.chats) {
                    ...
                    const activeChats = cache.chats.filter(
                        (c: any) =>
                            c.id.endsWith("@g.us") ||
                            (c.conversationTimestamp &&
                                (c.conversationTimestamp?.low ||
                    ...
                    if (activeChats.length < cache.chats.length) {
                        log(
                            "SYNC",
                            `Cleaned ${cache.chats.length - activeChats.length} polluted chats`,
                        );
                        this.save();
                    }
                }
                log(
                    "SYNC",
                    `Restored ${this.contacts.size} contacts, ${this.chats.size} chats (mappings: ${this.lidToPn.size})`,
                );
            }
        } catch (e: any) {
    ...
    public updateMappings(items: any[]) {
        ...
        for (const item of items) {
            ...
            if (item.id.includes("@lid") && item.phoneNumber) {
                const pn = jidNormalizedUser(
                    item.phoneNumber.includes("@s.whatsapp.net")
                        ? item.phoneNumber
            ...
            // Reverse mapping: PN has a linked lid field
                ...
                const lid = item.lid.includes("@lid")
                    ? item.lid
                    : item.lid + "@lid";
        ...
        if (count > 0) log("SYNC", `Mapped ${count} new LID/PN pairs`);
    }
    ...
    public async resolvePN(jid: string, sock: any = null): Promise<string> {
        ...
        if (sock?.signalRepository?.lidMapping) {
            try {
                ...
                if (pn) {
                    const fullPn = jidNormalizedUser(
        ...
        for (const [c_jid, c_info] of this.contacts.entries()) {
            ...
            if (
                c_info.lid &&
                (c_info.lid === jid ||
                    c_info.lid.includes(extractJidId(jid))) &&
                c_jid.includes("@s.whatsapp.net")
            ) {
    ...
    public async resolveLID(
        ...
        if (sock?.signalRepository?.lidMapping) {
            try {
                ...
                log(
                    "SYNC",
                    `Failed to get LID for PN ${normalizedJid}: ${e.message}`,
                );
            }
        }

        const contact =
            this.contacts.get(normalizedJid) || this.contacts.get(jid);
        if (contact?.lid) {
            const lid = contact.lid.includes("@lid")
                ? contact.lid
                : contact.lid + "@lid";
            this.pnToLid.set(normalizedJid, lid);
            this.lidToPn.set(lid, normalizedJid);
    ...
    public async getRelatedJids(
        ...
        for (const [c_jid, c_info] of this.contacts.entries()) {
            ...
            if (
                jid.includes("@lid") &&
                c_info.lid &&
                (c_info.lid === jid || c_info.lid.includes(extractJidId(jid)))
            ) {
                related.add(normalizedCjid);
            }
            if (
                normalizedCjid === jidNormalizedUser(jid) &&
                c_info.phoneNumber
            ) {
                related.add(
                    jidNormalizedUser(c_info.phoneNumber + "@s.whatsapp.net"),
                );
            }
        }
    ...
    public async getAggregatedChats(sock: any): Promise<any[]> {
        ...
        const getBaseId = (id: string): string | null => {
            if (!id || blockedDomains.some((domain) => id.endsWith(domain)))
                return null;
        ...
        const isGarbageName = (s: string) =>
            !s || s.trim() === "~" || isNumeric(s);
        ...
        for (const [id, contact] of this.contacts.entries()) {
            ...
            const preferredName = isStrictlySaved
                ? savedName
                : bizName || (!isGarbageName(pushName) ? pushName : "");
        ...
        const cleanedResult = allResults.filter((c) => {
            ...
            if (
                sock?.monitored?.has?.(c.id) ||
                sock?.monitored?.has?.(extractJidId(c.id))
            )
                return true;
            ...
            const strictlyUseful =
                isUseful && (c.isGroup || isSaved || isMonitored || hasHistory);
        ...
        log(
            "SYNC",
            `Saved ${cleanedResult.length} chats (discarded ${allResults.length - cleanedResult.length} shadows)`,
        );
