...
# Terminal 2 — frontend (Vite HMR on :5173, proxies /api and /ws to :3000)
cd web && bun run dev
```
...
- **Frontend build output:** `bun run build` outputs to `public/` (served by the backend). During dev, Vite on `:5173` proxies API calls to the backend on `:3000`.
