...
## 2026-05-19 - N+1 I/O in Fallback Lookups
**Learning:** Found a critical performance anti-pattern. When attempting to eliminate an N+1 database query inside a mapping loop (`allChats.map`) by replacing it with an inline asynchronous file system check (`Bun.file(fullPath).exists()`), it introduced an N+1 I/O bottleneck. This causes severe I/O spikes and can exhaust file descriptors, rendering it worse than the database lookup. Furthermore, removing the fallback entirely breaks functionality.
**Action:** When addressing N+1 queries inside mapping loops where a batch query is already performed (e.g., `profilePics[c.id]`), the proper optimization is to either ensure the initial batch query retrieves all necessary data or to introduce an LRU cache inside the fallback method, rather than substituting it with concurrent file I/O checks or deleting the fallback entirely.
