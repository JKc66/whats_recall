...
## 2024-05-30 - Fix WebSocket Session Authorization Bypass
**Vulnerability:** The Hono/Bun WebSocket implementation accepted connections based on a valid session, but failed to invalidate them if the session expired or the user logged out.
**Learning:** WebSocket connections do not automatically close when the session (e.g. cookie-based auth token) expires. This allows unauthorized access to broadcasted sensitive data on previously opened, now-expired connections.
**Prevention:** Always pass the session tracking information (like token and fingerprint) to `server.upgrade` context data. Periodically re-evaluate the session validity in long-running processes (e.g., broadcasts or pings) using the `verifySession` helper, and explicitly call `ws.close()` on expired or unauthenticated sessions.
